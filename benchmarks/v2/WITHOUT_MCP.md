# Benchmark Execution: WITHOUT MCP

## Pre-requisitos

1. **CRÍTICO: Desactivar Corbat MCP**:
   ```bash
   claude mcp remove corbat
   ```

2. Verificar que NO está activo:
   ```bash
...
4. **Guardar resultados** - En `results/without-mcp/scenario-XX/`
...
2. **NO dar contexto adicional** - Solo el prompt, nada más
3. **NO mencionar arquitectura** - El LLM debe decidir solo
4. **NO pedir patrones específicos** - Dejar que genere "vanilla"
5. **Guardar respuesta completa** - Incluyendo explicaciones del LLM

## Verificación de Aislamiento

Para garantizar que el benchmark es válido:

1. Los prompts deben ser **idénticos** a WITH_MCP.md
2. Verificar con diff:
   ```bash
   diff <(grep -A2 "^###" WITH_MCP.md) <(grep -A2 "^###" WITHOUT_MCP.md)
   # Debe mostrar diferencias solo en headers, no en prompts
   ```
